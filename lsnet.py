...
def _create_lsnet(variant, pretrained=False, **kwargs):
    ...
    model = build_model_with_cfg(
        ...
        variant,
        pretrained,
        pretrained_cfg=default_cfgs[variant],  # 使用 pretrained_cfg 而不是 default_cfg
        **kwargs,
    )
    return model
...
if __name__ == '__main__':
    ...
    # 1. 创建模型
    model = lsnet_t(num_classes=1000, pretrained=False).cuda()

    # 2. 加载本地权重
    checkpoint = torch.load('/media/tbb/shuju/Backbone_PTH/lsnet_t.pth')
    ...
    print(f"✓ 加载成功！输出形状: {[o.shape for o in output]}")