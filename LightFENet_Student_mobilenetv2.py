...
from KDNet.MobileNetV2 import mobilenet_v2
...
class Decoder(nn.Module):
    ...
    def __init__(self, in1, in2, in3, in4, in5):
        ...
        self.bcon5 = nn.Sequential(
            Up(scale_factor=2, mode='bilinear', align_corners=True),
            BasicConv2d(in5, in5, kernel_size=3, stride=1, padding=1)
        )
        self.bcon4 = BasicConv2d(in4, in5, kernel_size=3, stride=1, padding=1)
        ...
        self.bcon5_4 = nn.Sequential(
            Up(scale_factor=2, mode='bilinear', align_corners=True),
            BasicConv2d(in5 * 2, in4, kernel_size=3, stride=1, padding=1)
        )
        ...
        self.conv_d0 = nn.Sequential(
            Up(scale_factor=2, mode='bilinear', align_corners=True),
            BasicConv2d(in4, in3, kernel_size=3, stride=1, padding=1)
        )
        ...
        self.conv_d3 = BasicConv2d(in1 * 2, in1, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, f):
        ...
        d54 = self.bcon5_4(torch.cat((f[4], f[3]), 1))
        d43 = self.bcon4_3(torch.cat((d54, f[2]), 1))
        d32 = self.bcon3_2(torch.cat((d43, f[1]), 1))
        d21 = self.bcon2_1(torch.cat((d32, f[0]), 1))
        out = d21

        d54 = self.conv_d0(d54)
        d43 = torch.cat((d54, d43), dim=1)
...
class LightFENet(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        self.decoder = Decoder(16, 24, 32, 96, 320)
